...
public class VirtualMachineServerController extends Observable implements EventObserver {
    ...
    private org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(VirtualMachineServerController.class);
    ...
    private void addVirtualMachine(Element event) {
        ...
        for (VirtualMachineServer vmServer : virtualMachineServers) {
            if (vmServer.isLocatedAt(address, port)) {
                ...
                String vmid = event.attributes.get("vmid");
                //VirtualMachine vm = new VirtualMachine(vmServer, vmpath, vmusername, vmpassword, vmcapturepath);
                VirtualMachine vm = new VirtualMachine(vmServer, vmpath, vmusername, vmpassword, vmcapturepath, vmid);
    ...
    public LinkedList<VirtualMachineServer> getVirtualMachineServers() {
        return virtualMachineServers;
    }

    public void setVirtualMachineServers(LinkedList<VirtualMachineServer> virtualMachineServers) {
